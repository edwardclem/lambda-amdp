 ...
 */
public class TemplateCoarseValidationGenlex<DI extends ILabeledDataItem<Sentence, ?>> extends
	...
	private final IValidator<DI, LogicalExpression> validator;

	private final double margin;

	private final int									parsingBeam;

	protected TemplateCoarseValidationGenlex(
			...
			IValidator<DI, LogicalExpression> validator,
			int parsingBeam,
			double margin,
	...
	public ILexicon<LogicalExpression> generate(DI dataItem,
		...
		// Pre-generation parse to get highest scoring valid parse.
		//modified with JointTemplateCoarseGenlex.java in SPF
		final IParserOutput<LogicalExpression> preModelParseOutput = parser
		...
		for (final IDerivation<LogicalExpression> parse : preModelParseOutput.getAllDerivations()) {
			if (validator.isValid(dataItem, parse.getSemantics())) {
					final List<? extends IDerivation<LogicalExpression>> maxParses = preModelParseOutput.
							getMaxDerivations(e -> parse.getSemantics().equals(e.getSemantics()));
					for (final IDerivation<LogicalExpression> baseParse : maxParses) {
						if (bestValidScore == null  || baseParse.getScore() > bestValidScore) {
		...
		final IParserOutput<LogicalExpression> parserOutput = parser.parse(
				dataItem.getSample(), model.createDataItemModel(dataItem.getSample()), false,
	...
	public static class Builder<DI extends ILabeledDataItem<Sentence, ?>> {
		...
		public TemplateCoarseValidationGenlex<DI > build() {
			final GenerationRepository repository = new GenerationRepository();

			return new TemplateCoarseValidationGenlex<DI>(
					repository.setConstants(constants),
					repository.setConstants(createAbstractConstants()),
					maxTokens, parser, validator,parsingBeam, margin, origin, mark);
		}
	...
	public static class Creator<DI extends ILabeledDataItem<Sentence, ?>>
			implements IResourceObjectCreator<TemplateCoarseValidationGenlex<DI>> {

		private final String type;

		public Creator() {
			this("genlex.template.coarse.validation");
		}
		...
		@Override
		public TemplateCoarseValidationGenlex<DI> create(Parameters params,
			...
			final Builder<DI> builder = new Builder<DI>(
					...
					(IValidator<DI, LogicalExpression>)repo.get(params.get("validator")),
					params.getAsDouble("margin"),
		...
		public ResourceUsage usage() {
			return ResourceUsage.builder(type, TemplateCoarseGenlex.class)
					...
					.addParam("validator", IValidator.class, "validator for pre-generation of parses.")
					.addParam("margin", Double.class, "margin for pre-genlex validation.")
					.build();
		}
